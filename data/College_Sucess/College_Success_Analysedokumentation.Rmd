...
# Regressions
```{r}
names(data_college_success)

model_1 <- lm(gpa ~ satm + satv, data = data_college_success)
model_2 <- lm(gpa ~ hss + hsm + hse, data = data_college_success)
model_3 <- lm(gpa ~ hss + hsm + hse + satm + satv, data = data_college_success)

sjPlot::tab_model(model_1, model_2, model_3, show.std = T, show.ci = F)
```
